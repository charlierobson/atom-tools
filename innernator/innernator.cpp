...
Read recording of an Atom program from a WAV file.
...
What it does with it is left as an exercise for the reader :)
I only wrote this to check my understanding of the atom tape format,
and I'm glad I did. There was at least one unexpected treat!
...
P.S. look out for the special bonus ninth cycle at the end of a byte's stop bit!
...
int main(int argc, char** argv)
{
	if (argc < 2)
	{
		std::cout << "INNERNATOR (STARCAT) V" << VERSION << std::endl;
		std::cout << std::endl;
		std::cout << "Usage: innernator wavfile[.wav]" << std::endl;
		std::cout << std::endl;
		std::cout << "Produces output like *cat when fed an Atom cassette image." << std::endl;
		std::cout << "More useful as source than exe! WAVs should be 16 bit, mono." << std::endl;
		return 1;
	}
	...
	while(!lastBlock)
	{
		...
		//
		BYTE atomData[256];
		memset(atomData, 123, 256);
		for (i = 0; i < atomTapeHeader.bytesInBlockMinus1 + 1; ++i)
		{
			if (!likeAKnife.getByte(atomData[i]))
			{
